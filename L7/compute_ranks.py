...
def is_reciprocal(graph,node,page,k):
    if k==0:
        if node==page and page in graph[node]:
            return True
        else:
            return False
    if page in graph[node]:
        return True
    for e in graph[node]:
        if is_reciprocal(graph,e,page,k-1):
            return True
    return False
...
def compute_ranks(graph,k):
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            for node in graph:
                if page in graph[node] and not is_reciprocal(graph,page,node,k):
