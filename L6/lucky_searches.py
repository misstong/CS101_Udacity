...
def ordered_search(index, ranks, keyword):
    if not keyword in index:
        return None
    urls=index[keyword]
    quicksort(urls,ranks,0,len(urls)-1) #least likey to most likely 
    return urls
    #ret=quick_sort(urls,ranks) # most to least
    #return ret
...
def quick_sort(urls,ranks):
    ...
    for i in range(1,len(urls)):
        if ranks[urls[i]]<=ranks[key]:
            worse.append(urls[i])
        else:
            better.append(urls[i])
    return quick_sort(better,ranks)+[key]+quick_sort(worse,ranks)

def partition(urls,ranks,low,high):
    pivot=urls[low]
    i=low
    j=high
    while i<j:
        while i<j and ranks[urls[j]]>ranks[pivot]:
            j-=1
        while i<j and ranks[urls[i]]<=ranks[pivot]: 
            i+=1
...
def quicksort(urls,ranks,low,high):
    if low>=high:
        return 
    p=partition(urls,ranks,low,high)
    quicksort(urls,ranks,low,p-1)
    quicksort(urls,ranks,p+1,high)
